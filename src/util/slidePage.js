(function($) {
    ...
    console.log($(opt.pageContainer))
    console.log($(opt.pageContainer).css('transition-duration'))
    ...
    window.slidePage = {
        ...
        'index': function(index){
            if(index>0&&index!=keyIndex+1){
                var direction = index>keyIndex?'next':'prev';
                index=parseInt(index)-1;
                var endheight = $(".item").eq(direction=='prev'?index-1:index).children().outerHeight();
                if((endheight-windowH)>20){
                    var offset = 1
                }else{
                    var offset = 0
                }
                if(direction=='next'){
                    for(var i= keyIndex;i<index;i++){
                        ...
                        isScroll(i+2,'next',offset)
                        slideScroll(i+2,'next')
                    }
                }else{
                    for(var i=keyIndex;i>=index+1;i--){
                        prevPage($(opt.pageContainer).eq(i));
                        isScroll(i,'prev',offset)
                        slideScroll(i,'prev')
                    }
                }
                keyIndex=index;
        ...
        'prev': function(){
            if(keyIndex>0){
                var item = $(opt.pageContainer).eq(keyIndex--)
                prevPage(item)
                isScroll(item.index(),'prev')
                slideScroll(item.index(),'prev')
                direction=false
            }
        },
        'fire':function(index){
    ...
    function pageActive(){
        if(opt.refresh&&direction&&opt.useAnimation){
            ...
            direction=='next'?$(opt.pageContainer).eq(keyIndex).find('.lazy').addClass('hide'):$(opt.pageContainer).eq(keyIndex).find('.lazy').addClass('hide');
        }
    }
    //-- 拆分url的参数
    ...
    function nextPage(item) {
        ...
        opt.before(item.index()+1,'next',item.index()+2);
    ...
    function prevPage(item) {
        ...
        opt.before(item.index()+1,'prev',item.index());
    ...
    function initDom(opt) {
        ...
        //-- 如果有大神知道怎么解决请fork或联系我qq：498270108 谢谢。
        if (!!opt.speed){
    ...
    //-- 判断滚动模式
    function isScroll(target,direction,offset){
    ...
    function initEvent(opt) {
        ...
        function wheelFunc(e){
            ...
            if(e.wheelDeltaY<0||e.wheelDelta<0||e.detail>0){
                slidePage.canNext&&direction&&slidePage.next();
            }else if(e.wheelDeltaY>0||e.wheelDelta>0||e.detail<0){
                slidePage.canPrev&&direction&&slidePage.prev();
            }
